...
async function loadUserProfile(userId) {
    try {
        const response = await fetch(`${globalVariables.apiUrl}profile/user/${userId}`);
        ...
        if (userProfile.success) {
            localStorage.setItem(stringStore.profilekey, JSON.stringify(userProfile.data[0]));
        }
        document.getElementById("profileImage").src = userProfile.data[0].purl ? `http://localhost:8000${userProfile.data[0].purl}` : "/Assets/images/user.png";
    } catch (error) {
...
function Navi() {
    window.location.assign("/UserDash/index.html#messages");
    navLinks.forEach(link => {
        //Detect current page and active the link
        if (currentUrl.includes(link.querySelector('a').href)) {
            navLinks.forEach(nav => nav.classList.remove('active'));
